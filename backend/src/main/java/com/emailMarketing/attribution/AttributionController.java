...
public class AttributionController {
    ...
    private final AttributionModelRepository modelRepo;
    private final RevenueAttributionRepository revenueRepo;
    private final EmailInteractionRepository interactionRepo;

    public AttributionController(UserRepository userRepo, AttributionService attributionService,
            AttributionModelRepository modelRepo, RevenueAttributionRepository revenueRepo,
            EmailInteractionRepository interactionRepo) {
    ...
    public ResponseEntity<?> shopifyOrdersPaid(@RequestBody Map<String, Object> body,
            @RequestHeader(value = "X-Shopify-Topic", required = false) String topic,
            @RequestHeader(value = "X-Shopify-Signature", required = false) String sig) {
        ...
        return ResponseEntity
                .ok(Map.of("status", "OK", "orderId", res.orderId(), "attributedCampaigns", res.attributedCampaigns()));
    }

    @PostMapping("/webhooks/woocommerce/order/completed")
    public ResponseEntity<?> wooOrderCompleted(@RequestBody Map<String, Object> body,
            @RequestHeader(value = "X-WC-Webhook-Source", required = false) String source) {
        String id = String.valueOf(body.getOrDefault("id", body.getOrDefault("order_id", "unknown")));
        @SuppressWarnings("unchecked")
        Map<String, Object> billing = (Map<String, Object>) body.getOrDefault("billing", Collections.emptyMap());
    ...
    public RevenueBreakdown campaignRevenue(@AuthenticationPrincipal UserDetails principal, @PathVariable Long id) {
        // Query revenue across models from repository (simplified: use
        // revenue_attribution direct)
        var all = revenueRepo.findByCampaignIdAndModelCode(id, "LAST_TOUCH"); // extend to other models with loops
        double total = all.stream().mapToDouble(r -> r.getAttributedAmount() == null ? 0 : r.getAttributedAmount())
                .sum();
    ...
    public java.util.List<AttributionModelDto> models() {
        return modelRepo.findAll().stream()
                .map(m -> new AttributionModelDto(m.getModelCode(), m.getDescription(), m.getDecayHalfLifeDays()))
                .toList();
    }

    record CustomEventRequest(String email, Long campaignId, String type, String variant, String utmSource,
            String utmMedium, String utmCampaign) {
    }

    @PostMapping("/attribution/custom-event")
    public ResponseEntity<?> customEvent(@AuthenticationPrincipal UserDetails principal,
            @RequestBody CustomEventRequest req) {
        Long userId = resolveUserId(principal);
        EmailInteraction ei = new EmailInteraction();
        ei.setUserId(userId);
        ei.setCampaignId(req.campaignId());
        ei.setEmail(req.email());
        ei.setEventType(req.type());
        ei.setEventTime(LocalDateTime.now());
        ei.setVariantCode(req.variant());
        ei.setUtmSource(req.utmSource());
        ei.setUtmMedium(req.utmMedium());
        ei.setUtmCampaign(req.utmCampaign());
        ei.setChannel("EMAIL");
    ...
    private Long resolveMerchantUser(Map<String, Object> payload) {
        return 1L;
    } // TODO: map store domain/api key to user id

    private Long resolveUserId(UserDetails principal) {
        return userRepo.findByUsername(principal.getUsername()).map(u -> u.getId()).orElseThrow();
    }
}
