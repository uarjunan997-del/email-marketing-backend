...
public class FailoverEmailService {
    ...
    public FailoverEmailService(List<EmailProvider> providers) {
    ...
    public EmailSendResult send(String to, String subject, String html) {
        ...
        for (EmailProvider p : providers) {
            ...
            if (r.success())
                return r;
            last = r;
            if (r.type() == EmailSendResult.ResultType.PERMANENT_FAILURE
                    || r.type() == EmailSendResult.ResultType.SUCCESS) {
                return r; // don't try further if permanent
            }
            // transient/config -> try next
        }
        return last == null
                ? new EmailSendResult(false, EmailSendResult.ResultType.TRANSIENT_FAILURE, "No provider attempted",
                        null, "none")
                : last;
    }
}
