...
public class KafkaConfig {
    ...
    @Bean
    public ConsumerFactory<String, EmailEventPayload> emailEventConsumerFactory() {
        JsonDeserializer<EmailEventPayload> jd = new JsonDeserializer<>(EmailEventPayload.class);
        jd.addTrustedPackages("*");
        return new DefaultKafkaConsumerFactory<>(consumerProps(), new StringDeserializer(), jd);
    }

    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, EmailEventPayload> emailEventListenerFactory() {
        ConcurrentKafkaListenerContainerFactory<String, EmailEventPayload> f = new ConcurrentKafkaListenerContainerFactory<>();
        f.setConsumerFactory(emailEventConsumerFactory());
        f.setConcurrency(2);
        return f;
    }
}
