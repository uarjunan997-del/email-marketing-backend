...
public class SendingQueueProcessor {
    ...
    private final com.emailMarketing.email.FailoverEmailService failoverEmailService;
    private final com.emailMarketing.metrics.EmailMetrics emailMetrics;

    public SendingQueueProcessor(CampaignService campaignService, CampaignRecipientRepository recipientRepository,
            com.emailMarketing.email.FailoverEmailService failoverEmailService,
            com.emailMarketing.metrics.EmailMetrics emailMetrics) {
    ...
    public void processBatch() {
        campaignService.processQueueBatch((item, ok) -> {
            ...
            try {
                ...
                switch (result.type()) {
                    case SUCCESS -> {
                        if (rec != null && "QUEUED".equals(rec.getStatus()))
                            rec.setStatus("SENT");
                        emailMetrics.increment("provider_send", "SUCCESS");
                        log.debug("send_success provider={} subject='{}'", result.provider(), item.getSubject());
                    }
                    case CONFIG_ERROR, PERMANENT_FAILURE -> {
                        if (rec != null)
                            rec.setStatus("FAILED");
                        emailMetrics.increment("provider_send", result.type().name());
                        log.warn("send_permanent_failure provider={} msg={}", result.provider(), result.message());
                        throw new PermanentEmailSendException("Permanent failure: " + result.message());
                    }
