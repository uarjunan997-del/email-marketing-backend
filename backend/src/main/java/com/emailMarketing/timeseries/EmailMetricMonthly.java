...
@Entity
@Table(name="email_metrics_monthly")
public class EmailMetricMonthly {
    ...
    @Embeddable
    public static class Key implements java.io.Serializable { private Long userId; private LocalDateTime bucketStart; public Long getUserId(){return userId;} public void setUserId(Long userId){this.userId=userId;} public LocalDateTime getBucketStart(){return bucketStart;} public void setBucketStart(LocalDateTime bucketStart){this.bucketStart=bucketStart;} @Override public int hashCode(){return java.util.Objects.hash(userId,bucketStart);} @Override public boolean equals(Object o){if(this==o)return true; if(!(o instanceof Key k))return false; return java.util.Objects.equals(userId,k.userId)&&java.util.Objects.equals(bucketStart,k.bucketStart);} }
}
