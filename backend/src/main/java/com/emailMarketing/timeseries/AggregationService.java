...
public class AggregationService {
    ...
    public void aggregateHourly(){
        ...
        jdbc.update("INSERT /*+ APPEND */ INTO email_metrics_hourly (user_id,campaign_id,bucket_start,sent_count,open_count,click_count,bounce_count,order_count,revenue_amount) " +
            "SELECT e.user_id, e.campaign_id, TRUNC(e.event_time,'HH') bucket_start, " +
            ...
            "   SELECT user_id, campaign_id, TRUNC(created_at,'HH') bucket_start, COUNT(DISTINCT order_id) order_ct, SUM(attributed_amount) rev" +
            "   FROM revenue_attribution WHERE created_at BETWEEN ? AND ? GROUP BY user_id,campaign_id,TRUNC(created_at,'HH')" +
            ") ra ON ra.user_id=e.user_id AND ra.campaign_id=e.campaign_id AND ra.bucket_start=TRUNC(e.event_time,'HH') " +
            " WHERE e.event_time BETWEEN ? AND ? GROUP BY e.user_id, e.campaign_id, TRUNC(e.event_time,'HH')",
    ...
    public void aggregateDailyWeeklyMonthly(){
        ...
        jdbc.update("INSERT /*+ APPEND */ INTO email_metrics_daily (user_id,campaign_id,bucket_start,sent_count,open_count,click_count,bounce_count,order_count,revenue_amount) " +
            ...
            "   SELECT user_id, campaign_id, TRUNC(created_at) bucket_start, COUNT(DISTINCT order_id) order_ct, SUM(attributed_amount) rev" +
        ...
        jdbc.update("INSERT /*+ APPEND */ INTO email_metrics_weekly (user_id,campaign_id,bucket_start,sent_count,open_count,click_count,bounce_count,order_count,revenue_amount) " +
            ...
            " SUM(CASE WHEN e.event_type='BOUNCE' THEN 1 ELSE 0 END)," +
            " COALESCE(SUM(ra.order_ct),0), COALESCE(SUM(ra.rev),0) FROM email_events e LEFT JOIN (" +
            "   SELECT user_id, campaign_id, TRUNC(created_at,'IW') bucket_start, COUNT(DISTINCT order_id) order_ct, SUM(attributed_amount) rev" +
        ...
        jdbc.update("INSERT /*+ APPEND */ INTO email_metrics_monthly (user_id,campaign_id,bucket_start,sent_count,open_count,click_count,bounce_count,order_count,revenue_amount) " +
            ...
            " SUM(CASE WHEN e.event_type='BOUNCE' THEN 1 ELSE 0 END)," +
            " COALESCE(SUM(ra.order_ct),0), COALESCE(SUM(ra.rev),0) FROM email_events e LEFT JOIN (" +
            "   SELECT user_id, campaign_id, TRUNC(created_at,'MM') bucket_start, COUNT(DISTINCT order_id) order_ct, SUM(attributed_amount) rev" +
    ...
    public void backfillAll(int days){
        ...
        jdbc.update("INSERT /*+ APPEND */ INTO email_metrics_hourly (user_id,campaign_id,bucket_start,sent_count,open_count,click_count,bounce_count,order_count,revenue_amount) " +
            "SELECT user_id, campaign_id, TRUNC(event_time,'HH') bucket_start, " +
            ...
            " 0,0 FROM email_events WHERE event_time >= ? GROUP BY user_id,campaign_id,TRUNC(event_time,'HH')",
    ...
    public void manualBackfill(Long userId, java.time.Instant start, java.time.Instant end){
        ...
        jdbc.update("INSERT /*+ APPEND */ INTO email_metrics_hourly (user_id,campaign_id,bucket_start,sent_count,open_count,click_count,bounce_count,order_count,revenue_amount) " +
            ...
            " COALESCE(SUM(ra.order_ct),0), COALESCE(SUM(ra.rev),0) FROM email_events e LEFT JOIN (" +
            "   SELECT user_id, campaign_id, TRUNC(created_at,'HH') bucket_start, COUNT(DISTINCT order_id) order_ct, SUM(attributed_amount) rev" +
            "   FROM revenue_attribution WHERE user_id=? AND created_at BETWEEN ? AND ? GROUP BY user_id,campaign_id,TRUNC(created_at,'HH')" +
            ") ra ON ra.user_id=e.user_id AND ra.campaign_id=e.campaign_id AND ra.bucket_start=TRUNC(e.event_time,'HH') " +
            " WHERE e.user_id=? AND e.event_time BETWEEN ? AND ? GROUP BY e.user_id,e.campaign_id,TRUNC(e.event_time,'HH')",
